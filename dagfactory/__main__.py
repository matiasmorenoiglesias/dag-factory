...
from dagfactory.codegen import generate_dag_block, generate_dags_file
...
def generate(
    ...
    yaml_files = list(yaml_file_dir.rglob("*.yaml")) + list(yaml_file_dir.rglob("*.yml"))
    if not yaml_files:
    ...
    for yaml_file in yaml_files:
        try:
            config = load_yaml_file(str(yaml_file))
            ...
            for dag_name in config:
                ...
                if not isinstance(config[dag_name], dict):
                    continue
                dag_config = deepcopy(config[dag_name])
